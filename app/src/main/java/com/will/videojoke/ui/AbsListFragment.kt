...
import com.will.libcommon.view.EmptyView
import com.will.videojoke.R
import com.will.videojoke.databinding.LayoutRefreshViewBinding
...
abstract class AbsListFragment<V,M:AbsViewModel<Int,V>> : Fragment(), OnRefreshListener,
    ...
    private fun init(){
        ...
        if (arguments.isNotEmpty()){
            ...
            //触发页面初始化数据加载的逻辑
            mViewModel?.pageData?.observe(viewLifecycleOwner, Observer<PagedList<V>>{ t ->
                mAdapter?.submitList(t)

            })

            //监听数据边界，已决定是否关闭上拉加载的动画
            mViewModel!!.boundaryPageData.observe(viewLifecycleOwner, Observer { it ->
                finishRefresh(it)
            })
        }
    }
     ...
     */
    abstract fun getAdapter():PagedListAdapter<V,RecyclerView.ViewHolder>
}